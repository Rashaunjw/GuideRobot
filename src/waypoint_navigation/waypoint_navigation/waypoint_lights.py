...
WAYPOINTS = {
    ...
    "Home Base": ([0.0, 0.0], TurtleBot4Directions.EAST),
    # Add more rooms here
}
...
def main():
    ...
    lightring_pub = node.create_publisher(LightringLeds, '/cmd_lightring', qos_profile_sensor_data)
    ...
    def navigate_to_room(room_name):
        if room_name in WAYPOINTS:
            ...
            set_lights(0, 0, 255)  # 🔵 Navigating
            navigator.startToPose(goal_pose)
            navigator.waitUntilNavComplete()

            print(f"Arrived at: {room_name}")
            set_lights(0, 255, 0)  # ✅ Arrived
            rclpy.spin_once(node, timeout_sec=1.0)

            return_to = "Home Base"
            if return_to in WAYPOINTS:
                return_coords, return_dir = WAYPOINTS[return_to]
                return_pose = navigator.getPoseStamped(return_coords, return_dir)
                print(f"Returning to: {return_to} at {return_coords}")
                set_lights(255, 255, 0)  # 🟡 Returning
                navigator.startToPose(return_pose)
                navigator.waitUntilNavComplete()

                print("Navigation complete.")
                set_lights(0, 255, 0)  # ✅ Back home
            else:
                print(f"No return location set for: {return_to}")
                set_lights(255, 0, 0)  # 🔴 Error
        else:
